...
void cmd_showvar(char *tr[], char *arg3[]){
    ...
    if(tr[0] == NULL){
        ...
        if((posVar = BuscarVariable(tr[0], env)) == -1){
            ...
            char *variable = strdup(arg3[posVar]);
...
void cmd_job(char *tr[], tListP LP){
    ...
    if(tr[0] == NULL){
        ...
        if(!strcmp(tr[0], "-fg") && tr[1] != NULL){
            ...
            waitpid(pid, &st, 0);
            //crear un delete at pid cuando este hecho vien el removeP
...
void execute(char *tr[], tList *L, tListM *LM, tListP *LP){
    ...
    //the first element of argv is the name of the executable 
    while(tr[i] != NULL && !strcmp(tr[i], "&") && tr[i][0] != '@'){
        
        argv[i-executablePos] = strdup(tr[i]);         
    ...
    if(envp[0] == NULL){//if no envoroment variables were specified then all of them are passed
    ...
    if(ret == -1){
        perror("ERROR");
            //the memory used for the allocation of the strings is freed
        i = 0;
        while(argv[i] != NULL){
            free(argv[i]);
            i++;
...
void cmd_execute(char *tr[], bool isExecute, tList *L, tListM *LM, tListP *LP){
    char *prio = checkPrio(tr);
    bool background = checkBackground(tr);
    int pid_son;
